...
int  sensorAnalogPin = 33;    // Select the Arduino input pin to accept the Sound Sensor's analog output 
int  sensorDigitalPin = 32;    // Select the Arduino input pin to accept the Sound Sensor's digital output
...
void setup()
{
  ...
  pinMode(BUILTIN_LED, OUTPUT);            // Define LED13 as an output port, to indicate digital trigger reached
}

void loop(){
  analogValue = analogRead(sensorAnalogPin); // Read the value of the analog interface A0 assigned to analog 
